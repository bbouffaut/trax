...
class ValueTrainTask(training.TrainTask):
  ...
  def value_batch(self, trajectory_batch):
    ...
    """
    if self._sync_at(self._step) or self._step == 1:
    ...
    values = np.array(self.value(trajectory_batch))
    ...
    # advantages are zero.
    targets = values[:, :adv_seq_len] + advantages
  ...
  def value(self, trajectory_batch):
    ...
    """
    # TODO(pkozakowski): Instead check if the model has been initialized, once
    # we have the means of checking it.
    if self._step == 0:
...
class ValueEvalTask(training.EvalTask):
  ...
  def __init__(self, train_task, n_eval_batches=1, head_selector=()):
    ...
    super().__init__(labeled_data, metrics, n_eval_batches=n_eval_batches)
